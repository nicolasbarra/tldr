...
- ipynb versions of QC and AG
    - Found in src/ipynb
- Data from classmate HITs
    - /data/raw_data_input/initial_hit_results
- Data from Turkers voting on summaries 
    - /data/raw_data_input/initial_hit_results
...
In order to find our our task on Amazon Mechanical Turk Sandbox you can use this [link](https://workersandbox.mturk.com/requesters/AAHT5IXE5NHA0/projects?ref=w_pl_prvw) (Note that some HITs may be completed, in which case, it is necessary to select another HIT in order to make a contribution. Additionally, note that all HITs have Elizabeth as the name of the requester for reference.).
